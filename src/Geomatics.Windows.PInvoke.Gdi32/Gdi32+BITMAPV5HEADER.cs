...
namespace PInvoke
{
    ...
    public struct BITMAPV5HEADER
    {
        ...
        {
            if (bV5SizeImage != 0)
                return (int) bV5SizeImage;
            else return CalculateSurfaceStride() * bV5Height;
        }
        ...
        public uint OffsetToPixels
        {
            get
            {
                if (bV5Compression == BitmapCompression.BI_BITFIELDS)
                {
        ...
        private int CalculateSurfaceStride()
        {
            int stride = ((((bV5Width * (int) bV5BitCount) + 31) & ~31) >> 3);
            return stride;
        }
    }
}