    ...
    public class ImageConversionService
    {
        ...
        public static void SetClipboardImage(Bitmap image, Bitmap imageNoTr, DataObject data)
        {
            ...
            using (MemoryStream dibMemStream = new MemoryStream())
            {
                ...
                global::System.Windows.Clipboard.SetDataObject(data, true);
            }
        }
        ...
        public static Byte[] ConvertToDib(Image image)
        {
            ...
            using (Bitmap bm32b = new Bitmap(image.Width, image.Height, PixelFormat.Format32bppArgb))
            {
                ...
                Int32 stride;
                bm32bData = ImageService.GetImageData(bm32b, out stride);
            }
            ...
            if (retrievedData.GetDataPresent("PNG"))
            {
                ...
                if (png_stream != null)
                    using (Bitmap bm = new Bitmap(png_stream))
                        clipboardimage = ImageService.CloneImage(bm);
            }
        ...
        public static Bitmap ImageFromClipboardDibV5(Byte[] dibBytes)
        {
            GCHandle handle = GCHandle.Alloc(dibBytes, GCHandleType.Pinned);

            BITMAPINFOHEADER bmih = (BITMAPINFOHEADER)Marshal.PtrToStructure(handle.AddrOfPinnedObject(), typeof(BITMAPINFOHEADER));

            if (bmih.IsDibV5)
            {
                BITMAPV5HEADER bmh = (BITMAPV5HEADER)Marshal.PtrToStructure(handle.AddrOfPinnedObject(), typeof(BITMAPV5HEADER));
                
                Bitmap bitmap = new Bitmap(bmh.bV5Width, bmh.bV5Height,
                    -(int)(bmh.SizeImage() / bmh.bV5Height),
                    (int)bmh.bV5BitCount == 32 ? PixelFormat.Format32bppArgb : PixelFormat.Format24bppRgb,
                    new IntPtr(handle.AddrOfPinnedObject().ToInt32() + bmh.OffsetToPixels +
                               (bmh.bV5Height - 1) * (int)(bmh.SizeImage() / bmh.bV5Height)));
        ...
        public static Bitmap ImageFromClipboardDib(Byte[] dibBytes)
        {
            ...
            try
            {
                ...
                Bitmap bitmap = ImageService.BuildImage(image, width, height, stride, fmt, null, null);
